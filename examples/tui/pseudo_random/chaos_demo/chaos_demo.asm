...
_start:
    ...
    mov     [buffer], rax       ; higher qword in high address
    mov     [buffer+8], rdx     ; lower qword in low address
...
qwordbin2hexascii:
    ...
    push    r15
    push    rbx
    ; save volatile registers xmm0, xmm1, xmm2
    movq    r10,xmm0                ;lower 64 bits in r10
    movhlps xmm0,xmm0               ;move higher 64 bits of xmm0
    movq    r11,xmm0                ;high 64 bits in r11

    movq    r12,xmm1                ;lower 64 bits in r12
    movhlps xmm1,xmm1               ;move higher 64 bits of xmm1
    movq    r13,xmm1                ;high 64 bits in r13

    movq    r14,xmm2                ;lower 64 bits in r14
    movhlps xmm2,xmm2               ;move higher 64 bits of xmm2
    movq    r15,xmm2                ;high 64 bits in r15
    ; start conversion with rax containing the qword to convert
    ...
    psllw   xmm1,4
    paddb   xmm0,xmm1
    movq    rdx,xmm0                ;lower 64 bits in rax
    movhlps xmm0,xmm0               ;move higher 64 bits of xmm0
    movq    rax,xmm0                ;high 64 bits in rdx
    bswap   rdx                     ;swap bytes in rdx (endianess correction)
    bswap   rax                     ;swap bytes in rax (endianess correction)
    ;restore registers
